...
public class TodoList_electron extends common_ops {
    ...
    public void Change() {
        str = "Smile to world:)";
        electron_flows.change(1, str);
        verifications.verify_condition(true,electron_flows.getTodoIndex(str) > 0, "");
    }
    ...
    public void Completed() {
        str = "Wake up";
        electron_flows.complete(str);
        verifications.is_elem_exists_without_wait(_todo.get_completed_todo(driver, electron_flows.getTodoIndex(str)), true, "Task completion did not work as expected.");
    }
